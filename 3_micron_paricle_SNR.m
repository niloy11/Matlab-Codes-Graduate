...
t4 = 1.5:1/fs:2; %time period to get baseline voltage
    ...
    for j=1: length(fin)
        ...
        r= -4*resistivity*d^3/(pi*D^4); %resistance change due to passing of cell
        ...
        C=conv(noisy_signal,signal_filter); %convolution with filter to do the match filtering
        ...
        SNR(j) = 10*log10((peak_amplitude)^2/power_noise); %finding power SNR
    end   
   
    figure
    plot(fin,SNR);
    
    xlabel('number of electrode fingers');
    ylabel('Power SNR(dB)');
    title('design space');
